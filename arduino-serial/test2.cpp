...
char* timeToString2(struct tm *t);
...
int main()
{
	...
	timer = time(NULL);
	t = localtime(&timer);
	char* timechar = timeToString(t);
	char* logname = name(timechar);
	char name[] = "Odroid on\n";
	...
	timer = time(NULL);
	t = localtime(&timer);
	char* timechar2 = timeToString2(t);
	static char now[50];
	sprintf(now, "%s%s", timechar2, " ");
	DataStore(now, logname);
